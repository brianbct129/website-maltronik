...
class purePajinate {
	constructor(options) {
		this.config = {
			...
			navLabelNext: 'Next',
			navLabelLast: 'Last',
			navLabelPrev: '<i class="fa fa-arrow-left"></i>',
			navLabelNext: '<i class="fa fa-arrow-right"></i>',
	...
	init() {
		...
		var next = !this.config.showPrevNext ? '' : '<li class="next_link ' + this.config.defaultClass + '"><a class="next" href="" aria-label="' + this.config.navAriaLabelNext + '" onclick="return false;">' + this.config.navLabelNext + '</a></li>';
			...
			switch (this.config.navOrder[i]) {
				...
				case "num":
					...
					while (number_of_pages > current_link) {
						...
						navigation_html += '<a class="page_link' + extra_class + ' ' + this.config.defaultClass + '" longdesc="' + current_link + '" href="" aria-label="' + (current_link + 1) + '" onclick="return false;">0<span>' + (current_link + 1) + '</span></a>';
						current_link++;
					}
					if (this.config.showPrevNext) {
